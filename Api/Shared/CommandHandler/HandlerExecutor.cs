...
public class HandlerExecutor
{
    public static async Task<IResult> RunHandler(HttpContext context, Type type)
    {
        ...
        if (instance.JsonSerializerSettings != null)
        {
            var json = JsonConvert.SerializeObject(new ApiResult<object>
            {
                Code = "",
                Status = "SUCCESS",
                Data = response,
                Message = ""
            }, instance.JsonSerializerSettings);
    ...
    private static async Task<object> GetFromQueryAsync(HttpContext context, ParameterInfo parameterInfo)
    {
        ...
        var x = context.Request.Query[parameterInfo.Name].ToString();
    ...
    {
        var validateResult = handlerInstance.Validate();
