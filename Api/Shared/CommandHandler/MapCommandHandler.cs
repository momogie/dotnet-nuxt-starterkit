...
public static class Extensions
{
    ...
    static void MapOpenApi(WebApplication app, Type handlerType, string group, string routePath, RouteHandlerBuilder builder)
    {
        ...
        var parameters = handlerType.GetConstructors().First().GetParameters().Where(p => scope.ServiceProvider.GetService(p.ParameterType) == null);
        ...
        if (routePath.StartsWith("/api/v1", StringComparison.OrdinalIgnoreCase))
        {
            var aa = routePath.Split("/");
            if (aa.Length > 1) builder.WithTags(aa[3].ToUpper());
        } 
        else if (routePath.StartsWith("/api", StringComparison.OrdinalIgnoreCase))
        {
            var aa = routePath.Split("/");
...
public static class OpenApiTypeMapper
{
    ...
    public static Type MapOpenApiPrimitiveTypeToSimpleType(this OpenApiSchema schema)
    {
        ...
        var type = (schema.Type, schema.Format?.ToLowerInvariant(), schema.Required) switch
        {
